...
%and the model contains loops in its dynamics.
...
maximum which will be eventually reached in such case. %in the case of %an endless loop.
...
\hline
 Exploration method & Complete depth first search & {\bf Iterative} Breadth first search \\
\hline
\end{tabular}
\vspace*{4pt}
...
\end{table*}
 ...
 %Avec ASP-Thomas on doit connaitre apriori la bound du check.
 %Si on peut avoir une bonne aproximation safe de la taille du chemin on a des résultat efficace.
 %Par contre si la bound est incconue, en choissisant une trop petite on peut loupé une reachabilité.
 %Au contraire avec une bound trop grande on va impacter les performances de façon significative.
 %Sur le benchmark <nomdubenchmark> l'objectif est atteint avec un chemin de taille 18.
 %En fixant une bound à 21 on termine en moins d'une seconde. En fixant une bound à 100 on dépasse la minute.
